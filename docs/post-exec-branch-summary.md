...
The wrapper/replay path is implemented, but some downstream Go decoding paths still do not understand the synthetic `0x7d` post-exec transaction in all historical payload/block handling flows.

That limitation appears to be outside the main Rust wrapper architecture and remains follow-up integration work.
