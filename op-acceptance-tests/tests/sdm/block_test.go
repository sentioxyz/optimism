...
func mustFindRepeatedSlotBlock(
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		for i, ptx := range plannedTxs {
			...
			itx := includedTx{receipt: receipt, txIndex: i, blockNum: receipt.BlockNumber.Uint64()}
			included = append(included, itx)
...
func TestSDMEnabledCanonicalGasAccounting(gt *testing.T) {
	...
	t.Require().NotEmpty(payload.Entries, "post-exec payload must be non-empty for repeated-slot workload")
...
func TestSDMRepeatedSlotDedupAndRefundCapBehavior(gt *testing.T) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if receipts[0].BlockNumber.Uint64() != receipts[1].BlockNumber.Uint64() ||
			receipts[2].BlockNumber.Uint64() != receipts[3].BlockNumber.Uint64() {
			t.Logger().Warn("slot-touch workload pairs split across blocks; retrying",
				"attempt", attempt,
				"sameWarmBlock", receipts[0].BlockNumber.Uint64(),
				"sameClaimBlock", receipts[1].BlockNumber.Uint64(),
				"manyWarmBlock", receipts[2].BlockNumber.Uint64(),
				"manyClaimBlock", receipts[3].BlockNumber.Uint64())
			continue
		}

		sameRefund := getOPGasRefund(t, sys.L2EL, receipts[1].TxHash)
		sameReplay := replayBlockWithSDM(t, sys.L2EL, receipts[1].BlockNumber.Uint64())
		...
		manyReplay := replayBlockWithSDM(t, sys.L2EL, receipts[3].BlockNumber.Uint64())
