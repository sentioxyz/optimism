...
pub mod block;
pub use block::{OpBlockExecutionCtx, OpBlockExecutor, OpBlockExecutorFactory, SDMExecutionMode};
...
pub struct OpEvm<DB: Database, I, P = OpPrecompiles, Tx = OpTransaction<TxEnv>> {
    inner: op_revm::OpEvm<
        OpContext<DB>,
        sdm::PostExecCompositeInspector<I>,
        EthInstructions<EthInterpreter, OpContext<DB>>,
        P,
    >,
    inspect: bool,
    last_tx_warming_savings: u64,
    last_tx_warming_events: Vec<sdm::WarmingRefundEvent>,
    _tx: PhantomData<Tx>,
}
...
impl<DB: Database, I, P, Tx> OpEvm<DB, I, P, Tx> {
    ...
    pub fn new(
        ...
        Self {
            inner: op_revm::OpEvm(revm::context::Evm {
                ctx,
                inspector: sdm::PostExecCompositeInspector::new(inspector),
                instruction,
                precompiles,
                frame_stack,
            }),
    ...
    pub fn begin_post_exec_tx(&mut self, ctx: sdm::PostExecTxContext) {
    ...
    /// transaction.
    pub fn take_last_tx_warming_events(&mut self) -> Vec<sdm::WarmingRefundEvent> {
    ...
    pub fn take_last_post_exec_tx_result(&mut self) -> sdm::PostExecExecutedTx {
        let refund_total = self.take_last_tx_warming_savings();
        let refund_events = self.take_last_tx_warming_events();
        sdm::PostExecExecutedTx { refund_total, refund_events }
    }
}
