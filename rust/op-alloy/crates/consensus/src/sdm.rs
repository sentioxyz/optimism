//! Post-execution synthetic transaction types.
...
/// Type byte for the synthetic post-execution transaction.
pub const POST_EXEC_TX_TYPE_ID: u8 = 0x7D;

/// Per-transaction gas refund entry within a [`SDMPayload`].
...
/// Payload for the synthetic post-execution transaction.
#[derive(Debug, Clone, PartialEq, Eq, Hash, Default)]
#[cfg_attr(feature = "arbitrary", derive(arbitrary::Arbitrary))]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[cfg_attr(feature = "serde", serde(rename_all = "camelCase"))]
...
/// Synthetic post-execution transaction carrying a [`SDMPayload`].
#[derive(Debug, Clone, PartialEq, Eq, Hash, Default)]
#[cfg_attr(feature = "arbitrary", derive(arbitrary::Arbitrary))]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[cfg_attr(
...
impl TxPostExec {
    ...
    pub fn size(&self) -> usize {
        core::mem::size_of::<SDMPayload>() +
            self.input.len() +
            self.payload.entries.len() * core::mem::size_of::<SDMGasEntry>()
    }
...
impl TryFrom<TxPostExecSerdeHelper> for TxPostExec {
    ...
    fn try_from(value: TxPostExecSerdeHelper) -> Result<Self, Self::Error> {
        ...
        let payload = if let Some(input) = value.input {
            SDMPayload::from_rlp_bytes(input.as_ref())
...
/// Build a post-execution transaction from refund entries.
pub fn build_post_exec_tx(entries: Vec<SDMGasEntry>) -> TxPostExec {
    TxPostExec::new(SDMPayload { version: 1, entries })
}
...
pub fn extract_post_exec_payload_from_tx(tx: &TxPostExec) -> Option<SDMPayload> {
