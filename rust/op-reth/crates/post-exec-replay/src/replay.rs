use crate::types::{
    ...
    PostExecReplayRefundKind, PostExecReplaySummary, PostExecReplayTx, SDMExecutionMode,
};
use alloy_consensus::{
    Block as AlloyBlock, BlockBody, BlockHeader, TxReceipt, Typed2718, transaction::TxHashRef,
};
use op_alloy_consensus::{
    OpTransaction, POST_EXEC_TX_TYPE_ID, SDMGasEntry, SDMPayload, build_post_exec_tx,
};
use reth_evm::{Database, execute::BlockExecutor};
use reth_execution_errors::BlockExecutionError;
use reth_optimism_evm::{
    ConfigureSDMEvm, PostExecExecutorExt, WarmingRefundEvent, WarmingRefundKind,
};
...
pub enum PostExecReplayError {
    ...
    UnsupportedMode(SDMExecutionMode),
    /// Execution failed.
    #[error(transparent)]
...
fn into_replay_payload(payload: SDMPayload) -> PostExecReplayPayload {
...
{
    if config.mode != SDMExecutionMode::CounterfactualEnabled {
    ...
    let mut executor = evm_config
        .post_exec_executor_for_block(
            ...
            reth_optimism_evm::SDMExecutionMode::Produce,
        )
    ...
    Ok(PostExecReplayBlock {
        ...
        synthesized_payload_bytes: build_post_exec_tx(replay_entries).payload.to_rlp_bytes(),
...
mod tests {
    ...
    fn strips_post_exec_tx_and_preserves_original_indexes() {
        let deposit: OpTransactionSigned =
            OpTxEnvelope::Deposit(TxDeposit::default().seal_slow()).into();
        let user = user_tx();
        let post_exec: OpTransactionSigned =
            OpTransactionSigned::PostExec(build_post_exec_tx(vec![]).seal_slow());
    ...
    fn normalize_block_extracts_embedded_payload_and_post_exec_index() {
        ...
        let post_exec: OpTransactionSigned =
            OpTransactionSigned::PostExec(build_post_exec_tx(payload_entries.clone()).seal_slow());
        ...
        assert_eq!(normalized.embedded_payload.unwrap().entries, payload_entries);
    ...
    fn build_payload_map_reports_invalid_targets_and_duplicates() {
        ...
        let payload = op_alloy_consensus::SDMPayload {
            version: 1,
            entries: vec![
    ...
    fn compare_refunds_detects_tampered_payload_and_receipt_mismatches() {
        let config = PostExecReplayConfig {
            mode: SDMExecutionMode::CounterfactualEnabled,
