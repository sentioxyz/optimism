...
use reth_optimism_evm::{ConfigureSDMEvm, PostExecExecutorExt, SDMExecutionMode};
...
fn build_post_exec_recovered_tx(entries: Vec<SDMGasEntry>) -> Recovered<OpTransactionSigned> {
    let post_exec_tx = build_post_exec_tx(entries);
        ...
        if ctx.builder_config.sdm_enabled {
            ...
            if !entries.is_empty() {
                ...
                match builder.execute_transaction(post_exec_recovered) {
                    ...
                    Err(err) => {
                        warn!(target: "payload_builder", %err, "post-exec tx execution failed, skipping");
                    }
                }
            }
        }
    ...
    Attrs: OpAttributes<Transaction = TxTy<Evm::Primitives>>,
{
    ...
    pub fn block_builder<'a, DB: Database>(
        ...
        impl BlockBuilder<
            ...
            Executor: BlockExecutorFor<'a, Evm::BlockExecutorFactory, DB> + PostExecExecutorExt,
        > + 'a,
        PayloadBuilderError,
    > {
        self.evm_config
...
mod tests {
    ...
    fn build_post_exec_recovered_tx_wraps_entries_in_post_exec_tx() {
        ...
        let recovered = build_post_exec_recovered_tx(entries.clone());
        ...
        assert_eq!(tx.inner().payload.entries, entries);
    }
}
